/*
  ...
  Default setup portal credentials:
    username: admin
    password: (auto-generated, shown on Serial at boot)
*/
...
// ===== Mini OLED 0.96" Hologram Settings (SSD1306 I2C) =====
// Uncomment below if Adafruit_SSD1306 library is installed in Arduino IDE
// #define USE_OLED_HOLOGRAM 1

#ifdef USE_OLED_HOLOGRAM
#include <Wire.h>
...
String oledState = "STANDBY";
#endif

// ===== Audio stream settings =====
const int MIC_PIN = 34;
const int RELAY_PIN = 26;           // Default GPIO pin for Relay Module
const int OLED_SDA = 21;            // I2C SDA pin for 0.96" OLED
const int OLED_SCL = 22;            // I2C SCL pin for 0.96" OLED
...
// Password is auto-generated on first boot to ensure uniqueness
// Default is overridden in generateSetupPass()
...
void drawMiniHologramQuad(String stateStr) {
#ifdef USE_OLED_HOLOGRAM
  ...
  // Draw 4-quadrant Arc Reactor circles for Pepper's Ghost Mini Pyramid
  display.drawCircle(cx, cy - r, 6, SSD1306_WHITE); // Top
  display.drawCircle(cx, cy + r, 6, SSD1306_WHITE); // Bottom
  display.drawCircle(cx - r, cy, 6, SSD1306_WHITE); // Left
  display.drawCircle(cx + r, cy, 6, SSD1306_WHITE); // Right
  display.fillCircle(cx, cy, 3, SSD1306_WHITE);     // Center Core
...
void handleIncomingServerCommands() {
    ...
    if (strcmp(cmd, "relay") == 0) {
      ...
      if (state) {
        if (strcmp(state, "on") == 0) {
          ...
          drawMiniHologramQuad("RELAY: ON");
        } else if (strcmp(state, "off") == 0) {
          digitalWrite(RELAY_PIN, LOW);
          Serial.println("[relay] Switched OFF");
          drawMiniHologramQuad("RELAY: OFF");
        }
      }
    } else if (strcmp(cmd, "display") == 0) {
      const char* displayState = doc["state"];
      if (displayState) {
        drawMiniHologramQuad(displayState);
      }
    }
  }
}
...
void setup() {
  ...
  if (display.begin(SSD1306_SWITCHCAPVCC, SCREEN_ADDRESS)) {
    display.clearDisplay();
    drawMiniHologramQuad("STANDBY");
    Serial.println("[oled] SSD1306 OLED Hologram display initialized.");
  } else {
    Serial.println("[oled-warn] SSD1306 allocation failed");
  }
#endif
