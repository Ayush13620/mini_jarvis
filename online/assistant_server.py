...
try:
    from hologram_hud import HologramHUD
except Exception:
    HologramHUD = None
...
# Project A.R.C. Groq & HUD Extensions
...
ENABLE_HUD = os.getenv("ENABLE_HUD", "true").lower() == "true"
...
IOT_TOOLS = [
    {
        ...
        "function": {
            "name": "toggle_relay",
            "description": "Control IoT hardware appliances connected to the ESP32 relay module.",
            "parameters": {
                "type": "object",
                "properties": {
                    "device": {
                        "type": "string",
                        "description": "Target device (e.g. fan, light, lamp, relay, switch, AC)",
                    },
                    "state": {
                        "type": "string",
                        "enum": ["on", "off"],
...
# Constants for TTS and HUD
TTS_AMPLITUDE_BASE = 0.3
TTS_AMPLITUDE_RANGE = 0.7
TTS_AMPLITUDE_FREQ = 20.0
AUTO_TUNE_ENERGY_MULTIPLIER = 1.25
AUTO_TUNE_MAX_ENERGY = 20000.0
...
class MiniJarvis:
    def __init__(self):
        ...
        if ENABLE_HUD and HologramHUD is not None:
            try:
                self.hud = HologramHUD()
                self.hud.start()
                print("[hud] Started Hologram HUD window.")
            except Exception as e:
                print(f"[hud] Could not start HUD: {e}")

        if GROQ_API_KEY and Groq is not None:
            self.groq_client = Groq(api_key=GROQ_API_KEY)
            print("[init] Groq client initialized.")
        ...
        if STT_PROVIDER == "local":
            ...
            if self.groq_client is None:
                raise RuntimeError("GROQ_API_KEY must be set in .env to use STT_PROVIDER=groq.")
        elif STT_PROVIDER == "openai":
            self._init_openai_client()
        else:
            raise RuntimeError("STT_PROVIDER must be 'local', 'groq', or 'openai'.")
        ...
        if TTS_PROVIDER == "local":
            ...
            if edge_tts is None:
                raise RuntimeError("edge-tts package not installed. Run `pip install -r requirements.txt`.")
        elif TTS_PROVIDER == "openai":
            self._init_openai_client()
        else:
            raise RuntimeError("TTS_PROVIDER must be 'local', 'edge-tts', or 'openai'.")
    ...
    def _start_tts_worker(self) -> None:
        ...
        self._tts_worker_thread = threading.Thread(target=_worker, daemon=True)
        self._tts_worker_thread.start()

    def _shutdown_tts_worker(self) -> None:
        """Signal the TTS worker thread to stop gracefully."""
        if TTS_PROVIDER == "local" and ASYNC_TTS and hasattr(self, '_tts_worker_thread'):
            try:
    ...
    def auto_tune_after_noise(self, seg_ms: int) -> None:
        ...
        self.segmenter.energy_floor = min(
            self.segmenter.energy_floor * AUTO_TUNE_ENERGY_MULTIPLIER,
            AUTO_TUNE_MAX_ENERGY
        )
        print(
    ...
    def send_esp32_state(self, state: str, relay_info: str = "") -> None:
        with self._active_conn_lock:
            if self.active_conn:
                try:
                    payload = json.dumps({"command": "display", "state": state, "info": relay_info}) + "\n"
                    self.active_conn.sendall(payload.encode("utf-8"))
                except Exception:
                    pass

    def update_system_state(self, state: str, text: str = "", relay_info: str = "") -> None:
        if self.hud:
            self.hud.set_state(state, text=text, relay_info=relay_info)
        self.send_esp32_state(state, relay_info=relay_info)

    def execute_relay_command(self, device: str, state: str) -> str:
        msg = f"Relay {device} switched {state}."
        print(f"[iot-relay] {msg}")
        self.update_system_state("RELAY", relay_info=f"{device}: {state}")
        with self._active_conn_lock:
            if self.active_conn:
                try:
                    payload = json.dumps({"command": "relay", "device": device, "state": state}) + "\n"
                    self.active_conn.sendall(payload.encode("utf-8"))
                    print(f"[tcp-dispatch] Sent to ESP32: {payload.strip()}")
                except Exception as e:
                    print(f"[tcp-dispatch-error] Could not send to ESP32: {e}")
        return msg

    def reply_text(self, user_text: str) -> str:
        if self.hud:
            self.hud.set_state("THINKING", text=user_text)

        self.chat_history.append({"role": "user", "content": user_text})
        self._trim_chat_history()

        if self.groq_client is not None and (GROQ_API_KEY or not USE_OLLAMA_CHAT):
            try:
                ...
                if msg.tool_calls:
                    for tool_call in msg.tool_calls:
                        if tool_call.function.name == "toggle_relay":
                            args = json.loads(tool_call.function.arguments)
                            device = args.get("device", "relay")
                            state = args.get("state", "on")
                            relay_result = self.execute_relay_command(device, state)
                            ...
                            text = (followup.choices[0].message.content or relay_result).strip()
                ...
                print(f"[groq-error] {e}. Falling back to default chat handler.")
    ...
    def _trim_chat_history(self) -> None:
        """Keep system prompt + last MAX_CHAT_HISTORY exchanges."""
        if len(self.chat_history) > MAX_CHAT_HISTORY + 1:
            self.chat_history = [self.chat_history[0]] + self.chat_history[1:][-MAX_CHAT_HISTORY:]
    ...
    def speak(self, text: str) -> None:
        ...
        self.update_system_state("SPEAKING", text=text)
        ...
        if TTS_PROVIDER == "local":
            ...
            try:
                ...
                print(f"[warn] Local TTS failed: {exc}")
            self.update_system_state("STANDBY")
            return

        if TTS_PROVIDER == "edge-tts":
            if edge_tts is None:
                print("[warn] edge-tts package not available.")
                return
            tmp_filename = None
            try:
                ...
                if pygame is not None:
                    try:
                        ...
                        while channel and channel.get_busy():
                            if self.hud:
                                self.hud.set_audio_amplitude(
                                    TTS_AMPLITUDE_BASE + TTS_AMPLITUDE_RANGE * math.sin(time.time() * TTS_AMPLITUDE_FREQ)
                                )
                            time.sleep(0.05)
                        played = True
                    except Exception as e:
                        print(f"[warn] pygame.mixer playback failed: {e}")

                if not played:
                    print(f"[tts] (edge-tts output generated, ready at {tmp_filename})")
            except Exception as exc:
                print(f"[warn] Edge-TTS failed: {exc}")
            finally:
                self.update_system_state("STANDBY")
                if self.hud:
                    self.hud.set_audio_amplitude(0.0)
    ...
    def handle_chunk(self, raw_chunk: bytes) -> None:
        ...
        try:
            ...
            self.update_system_state("TRANSCRIBING")
            ...
            self.update_system_state("THINKING")
...
def recv_auth_line(conn: socket.socket) -> Tuple[str, str, bytes]:
    """
    Parse auth handshake from client.

    Returns:
        (status, line, buffered_bytes)
        - status: AuthResult.TIMEOUT, EMPTY, PARTIAL, or OK
        - line: The auth line (without newline) if status is OK, else empty string
        - buffered_bytes: Remaining bytes after the newline
    """
    conn.settimeout(2.0)
    try:
        first = conn.recv(256)
        ...
        return AuthResult.TIMEOUT, "", b""
    conn.settimeout(None)

    if not first:
        return AuthResult.EMPTY, "", b""

    if b"\n" not in first:
        return AuthResult.PARTIAL, "", first

    line, _, rest = first.partition(b"\n")
    return AuthResult.OK, line.decode(errors="ignore").strip(), rest
...
def authorized(conn: socket.socket) -> Tuple[bool, bytes]:
    ...
    status, line, buffered = recv_auth_line(conn)
...
def _handle_client(conn: socket.socket, addr: tuple, bot: "MiniJarvis") -> None:
    ...
    # Set active connection with lock
    with bot._active_conn_lock:
        bot.active_conn = conn
